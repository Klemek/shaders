...
#define F1  preset(midi(29, 11), 0.33, 0.00, 0.00, 0.00)
#define P1  preset(midi(13, 12), 0.55, 0.28, 0.00, 0.00)
...
#define F3  preset(midi(31, 11), 1.00, 0.35, 0.00, 0.00)
#define P3  preset(midi(15, 12), 0.25, 0.00, 0.00, 0.00)
...
#define F4  preset(midi(00, 12), 0.05, 0.15, 0.00, 0.00)
#define P4  preset(midi(16, 12), 0.30, 0.35, 0.00, 0.00)
...
#define F5  preset(midi(01, 12), 0.90, 0.35, 0.00, 0.00)
#define P5  preset(midi(17, 12), 0.15, 0.18, 0.00, 0.00)
...
#define F6  preset(midi(02, 12), 0.45, 0.08, 0.00, 0.00)
#define P6  preset(midi(18, 12), 0.30, 1.00, 0.00, 0.00)
...
#define F7  preset(midi(03, 12), 1.00, 0.80, 0.00, 0.00)
#define P7  preset(midi(19, 12), 0.50, 0.80, 0.00, 0.00)
#define B71 preset(midi(03, 13), 1.00, 1.00, 0.00, 0.00)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    uv = butt(B71, lens(uv, F7, P7), uv);
    // B61 / P6 movement speed / F6 movement range
    uv = butt(B61, move(uv, P6, F6), uv);
    // B31 / P3 rotation speed
    uv = butt(B31, rotate(uv, P3 * .5), uv);
    // B51 / P5 - zoom / F5 - shape
    uv = butt(B51, pan(uv, P5 * 10, F5), uv);
    // B41 / F4 - wave repeat / P4 - wave speed
    uv = butt(B41, uv * cos(length(uv * F4 * 200) + iTime * P4 * 10), uv);
